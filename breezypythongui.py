...
class EasyFrame(Tkinter.Frame):
    ...
    def addButton(self, text, row, column,
                  ...
                  pressClick = lambda: None,
                  releaseClick = lambda: None):
        """Creates and inserts a button at the row and column,
        and returns the button."""
        button = Tkinter.Button(self, text = text,
                                command = command, state = state,
                                bg = bg, fg = fg)
        button.bind("<ButtonPress-1>", lambda e: pressClick())
        button.bind("<ButtonRelease-1>", lambda e: releaseClick())
        button.bind('<Button-2>', lambda e: rightClick())
        button.bind('<Button-3>', lambda e: rightClick())
        self.rowconfigure(row, weight = 1)
        self.columnconfigure(column, weight = 1)
